...
(function() {
    'use strict';
    var _vendorCSSPrefix, _vendorStylePropertyPrefix, _vendorTransformLookup, _pointerEventsPrefixed, _setPointerCapture, _releasePointerCapture, _lostPointerCapture, _trackPointerEvents, _pointerTypeTouch;
    ...
    var _transformPrefixes = {
        x: '',
        y: '0,'
    };
    var _transformSuffixes = {
        x: ',0' + (_useToggleableHardwareAcceleration ? ')' : ',0)'),
        y: (_useToggleableHardwareAcceleration ? ')' : ',0)')
    };
    ...
    FTScroller = function(domNode, options) {
        ...
        setDisabledInputMethods = function setDisabledInputMethods(disabledInputMethods) {
            ...
            for (i in _instanceOptions.disabledInputMethods) {
                ...
                if (_instanceOptions.disabledInputMethods[i] !== disabledInputMethods[i])
                    changed = true;
        ...
        _updateDimensions = function _updateDimensions(ignoreSnapScroll) {
            ...
            var targetPosition = {
                x: _lastScrollPosition.x,
                y: _lastScrollPosition.y
            };
        ...
        _updateSegments = function _updateSegments(scrollFinalised) {
            ...
            if (newSegment.x !== _activeSegment.x || newSegment.y !== _activeSegment.y) {
                ...
                _fireEvent('segmentwillchange', {
                    segmentX: newSegment.x,
                    segmentY: newSegment.y
                })
            }
            if (scrollFinalised) {
        ...
        _setAxisPosition = function _setAxisPosition(axis, position, animationDuration, animationBezier, boundsCrossDelay) {
            ...
            if (newPositionAtExtremity !== _scrollAtExtremity[axis]) {
                if (newPositionAtExtremity !== null) {
                    if (animationDuration) {
                        _timeouts.push(setTimeout(function() {
                            _fireEvent('reached' + newPositionAtExtremity, {
                                axis: axis
                            })
        ...
        _fireEvent = function _fireEvent(eventName, eventObject) {
            ...
            for (i = 0,
            l = _eventListeners[eventName].length; i < l; i = i + 1) {
                try {
        ...
        _onTouchStart = function _onTouchStart(startEvent) {
            ...
            if (_inputIdentifier) {
                for (i = 0,
                l = startEvent.touches.length; i < l; i = i + 1) {
        ...
        _onTouchEnd = function _onTouchEnd(endEvent) {
            ...
            if (endEvent.touches) {
                for (i = 0,
                l = endEvent.touches.length; i < l; i = i + 1) {
        ...
        _onMouseScroll = function _onMouseScroll(event) {
            ...
            if (!_instanceOptions.invertScrollWheel && ((_instanceOptions.scrollingX && !_instanceOptions.scrollingY && Math.abs(scrollDeltaX) < Math.abs(scrollDeltaY)) || (!_instanceOptions.scrollingX && _instanceOptions.scrollingY && Math.abs(scrollDeltaX) > Math.abs(scrollDeltaY)))) {
                return
            }
        ...
        _getBoundingRect = function _getBoundingRect(anElement) {
            ...
            return {
                left: x,
                top: y,
                width: anElement.offsetWidth,
                height: anElement.offsetHeight
            }
        }
        ;
        _initializeDOM();
        ...
        if (Object.defineProperties) {
            Object.defineProperties(_publicSelf, {
                'scrollHeight': {
                    get: function() {
                        return _metrics.content.y
                    },
                    set: function(value) {
                        throw new SyntaxError('scrollHeight is currently read-only - ignoring ' + value);
                    }
                },
                'scrollLeft': {
                    get: function() {
                        return -_lastScrollPosition.x
                    },
                    set: function(value) {
                        scrollTo(value, false, false);
                        return -_lastScrollPosition.x
                    }
                },
                'scrollTop': {
                    get: function() {
                        return -_lastScrollPosition.y
                    },
                    set: function(value) {
                        scrollTo(false, value, false);
                        return -_lastScrollPosition.y
                    }
                },
                'scrollWidth': {
                    get: function() {
                        return _metrics.content.x
                    },
                    set: function(value) {
                        throw new SyntaxError('scrollWidth is currently read-only - ignoring ' + value);
                    }
                },
                'segmentCount': {
                    get: function() {
                    ...
                    set: function(value) {
                        throw new SyntaxError('segmentCount is currently read-only - ignoring ' + value);
                    }
                },
                'currentSegment': {
                    get: function() {
                    ...
                    set: function(value) {
                        throw new SyntaxError('currentSegment is currently read-only - ignoring ' + value);
                    }
                },
                'contentContainerNode': {
                    get: function() {
                        return _contentParentNode
                    },
                    set: function(value) {
                        throw new SyntaxError('contentContainerNode is currently read-only - ignoring ' + value);
                    }
                }
            })
        }
...
(function() {
    ...
    CubicBezier.prototype.divideAtT = function(t) {
        ...
        return [new CubicBezier(left.p1.x,left.p1.y,left.p2.x,left.p2.y), new CubicBezier(right.p1.x,right.p1.y,right.p2.x,right.p2.y)]
    }
    ;
